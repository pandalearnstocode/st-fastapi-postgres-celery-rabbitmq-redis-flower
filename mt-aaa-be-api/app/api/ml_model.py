...
async def submit_model_train_job(train_payload: TrainModel):
    ...
    await ModelMetaData.objects.update_or_create(model_id=model_id, status ="submitted",model_name =model_name)
    train_model_job.delay(model_name, split_ratio, model_id)
    return {"model_id": model_id, "status": "submitted", "model_name":model_name}
...
async def update_model_train_job_status(status_payload: ModelMetaData):
    ...
    return {"model_id": model_id, "status": status, "model_name":model_name}
...
async def get_model_status(model_id: str):
    try:
        ...
        status_dict = {"model_id": model_id, "status": "unknown", "model_name": "unknown"}
    return {"model_id": status_dict['model_id'], "status": status_dict['status']}
...
async def aggregate_results(agg_payload: AggregateResults):
    ...
    if model_results['status'] == 'done':
        ...
        return {"model_id": model_id, "status": model_results['status']}